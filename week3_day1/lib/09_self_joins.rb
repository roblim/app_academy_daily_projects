...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT route_a.company, route_a.num
    FROM
      routes AS route_a
    JOIN
      routes AS route_b ON route_a.company = route_b.company AND route_a.num = route_b.num
    WHERE
      route_a.stop_id = 115 AND route_b.stop_id = 137;
...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes AS route_b ON route_a.company = route_b.company AND route_a.num = route_b.num
    JOIN
      stops AS stop_a ON route_a.stop_id = stop_a.id
    JOIN
      stops AS stop_b ON route_b.stop_id = stop_b.id
    WHERE
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT stop_b.name, route_b.company, route_b.num
    FROM
      routes AS route_a
    JOIN
      routes AS route_b ON route_a.company = route_b.company AND route_a.num = route_b.num
...
# SELECT
#   DISTINCT route_a.num, route_a.company, stop_b.name
# FROM
#   routes AS route_a
# JOIN
#   routes AS route_b ON route_a.company = route_b.company AND route_a.num = route_b.num
...
#   stops AS stop_b ON route_b.stop_id = stop_b.id
# WHERE
#   stop_a.name = 'Craiglockhart' AND stop_b.name IN
#   (
#   SELECT
#     DISTINCT stop_a.name
#   FROM
#     routes AS route_a
#   JOIN
#     routes AS route_b ON route_a.company = route_b.company AND route_a.num = route_b.num
