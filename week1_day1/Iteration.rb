...
def factors(num)
  factors = []
  num.times { |n| factors << (n + 1) if num % (n + 1) == 0 }
  factors
end
...
class Array
  def bubble_sort!(&prc)
    ...
    while idx < self.length - 1
      if self[idx] > self[idx+1] && !prc
        swap(idx)
        idx = 0
      elsif prc && prc.call(self[idx], self[idx+1]) == 1
        swap(idx)
        idx = 0
      else
        idx += 1
  ...
  def bubble_sort(&prc)
    new_arr = self.dup
    new_arr.bubble_sort!(&prc) if prc
    new_arr.bubble_sort! unless prc
    new_arr
  end
end
...
class Array
  ...
  def my_inject(&blk)
    ...
    (1...self.length).each do |idx|
      result = blk.call(result, self[idx])
    end
    result
  end
end
...
def concatenate(strings)
  strings.my_inject { |string, substring| string + substring}
end
